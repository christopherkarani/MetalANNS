...
- [ ] Phase 1–4 code exists: `BeamSearchCPU.swift`, `SearchGPU.swift`, `SearchResult.swift`, `NNDescentCPU.swift`, `NNDescentGPU.swift`, `Distance.metal`, `NNDescent.metal`, `Sort.metal` all present
- [ ] `git log --oneline` baseline verified (expect 17 commits before Phase 5 execution)
- [ ] Full test suite passes (36 tests, zero failures): `xcodebuild test -scheme MetalANNS-Package -destination 'platform=macOS' 2>&1 | grep -E '(passed|failed)'`
- [ ] Read implementation plan: `docs/plans/2026-02-25-metalanns-implementation.md` (lines 2515–2567, Tasks 16–18)
- [ ] Read this phase's prompt: `docs/prompts/phase-5-persistence.md` (detailed spec for Tasks 16–18)
...
- [ ] 16.1 — Create `Tests/MetalANNSTests/PersistenceTests.swift` — 3 tests using Swift Testing:
...
- [ ] 16.2 — **RED**: `xcodebuild test -scheme MetalANNS-Package -destination 'platform=macOS' -only-testing MetalANNSTests/PersistenceTests 2>&1 | grep -E '(PASS|FAIL|error:)'` → confirms FAIL
- [ ] 16.3 — Create `Sources/MetalANNSCore/IndexSerializer.swift`:
...
- [ ] 16.4 — **DECISION POINT (16.5)**: How to populate GraphBuffer from NNDescentCPU output `[[(UInt32, Float)]]` in tests. Options: (a) helper on GraphBuffer, (b) inline in test, (c) utility function. **Document approach in notes below.**
- [ ] 16.5 — **GREEN**: All 3 tests pass. Specifically confirm `saveAndLoadRoundtrip` produces identical search results.
- [ ] 16.6 — **REGRESSION**: All Phase 1–4 tests still pass (36 prior tests)
- [ ] 16.7 — **GIT**: `git add Sources/MetalANNSCore/IndexSerializer.swift Tests/MetalANNSTests/PersistenceTests.swift && git commit -m "feat: implement index serialization with binary file format"`
...
- [ ] 17.1 — Create `Tests/MetalANNSTests/IncrementalTests.swift` — 2 tests using Swift Testing:
  - `insertAndFindNew`:
...
- [ ] 17.2 — **RED**: `xcodebuild test -scheme MetalANNS-Package -destination 'platform=macOS' -only-testing MetalANNSTests/IncrementalTests 2>&1 | grep -E '(PASS|FAIL|error:)'` → confirms FAIL
- [ ] 17.3 — Create `Sources/MetalANNSCore/IncrementalBuilder.swift`:
...
- [ ] 17.4 — **DECISION POINT (17.5)**: How to search on GraphBuffer in tests. Options: (a) extract to arrays and use BeamSearchCPU, (b) use SearchGPU if GPU available, (c) write thin adapter. **Document approach in notes below.**
- [ ] 17.5 — **GREEN**: Both tests pass. Specifically confirm `insertRecallDegradation` shows < 5% degradation.
- [ ] 17.6 — **REGRESSION**: All prior tests still pass (36 + 3 from Task 16 = 39)
- [ ] 17.7 — **GIT**: `git add Sources/MetalANNSCore/IncrementalBuilder.swift Tests/MetalANNSTests/IncrementalTests.swift && git commit -m "feat: implement incremental vector insertion with local graph repair"`
...
- [ ] 18.1 — Create `Tests/MetalANNSTests/DeletionTests.swift` — 3 tests using Swift Testing:
  - `deletedNotInResults`:
...
- [ ] 18.2 — **RED**: `xcodebuild test -scheme MetalANNS-Package -destination 'platform=macOS' -only-testing MetalANNSTests/DeletionTests 2>&1 | grep -E '(PASS|FAIL|error:)'` → confirms FAIL
- [ ] 18.3 — Create `Sources/MetalANNSCore/SoftDeletion.swift`:
...
- [ ] 18.4 — **DECISION POINT (18.4)**: Should `filterResults` request extra results to compensate for deletions? Recommended: no — keep it a pure filter, caller handles ef adjustment. **Document decision in notes below.**
- [ ] 18.5 — **GREEN**: All 3 tests pass. Specifically confirm `deletedNotInResults` shows zero deleted IDs in results.
- [ ] 18.6 — **REGRESSION**: All prior tests still pass (36 + 3 + 2 from Tasks 16–17 = 41)
- [ ] 18.7 — **GIT**: `git add Sources/MetalANNSCore/SoftDeletion.swift Tests/MetalANNSTests/DeletionTests.swift && git commit -m "feat: implement soft deletion with filtered search results"`
